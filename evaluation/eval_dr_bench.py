...
from prompts.score_prompt_zh import generate_merged_score_prompt as zh_merged_score_prompt
from prompts.score_prompt_en import generate_merged_score_prompt as en_merged_score_prompt
...
# from utils.parse_html import html_to_message_content
...
CRITERIA_FILE = "data/drb/criteria.jsonl"
REFERENCE_FILE = "data/drb/reference.jsonl"
MAX_RETRIES = 10
...
class AIClient:
    ...
    def __init__(
        ...
        base_url: str | None = DEFAULT_UNIAPI_BASE_URL,
    ):
        self.api_key = api_key
        if not self.api_key:
            raise ValueError("Judge API key missing. Set UNIAPI_API_KEY, OPENAI_API_KEY, or pass --api_key_env.")
    ...
    def generate(self, user_prompt: str, system_prompt: str = "", model: Optional[str] = None) -> str:
        """
        Generate text using the configured judge API.
        """
        ...
        try:
            ...
            raise Exception(f"Failed to call judge API: {str(e)}")
        

    def generate_message(self, user_message: str, system_prompt: str = "", model: Optional[str] = None) -> str:
        """
        Generate text using the configured judge API.
        """
        model_to_use = model or self.model
...
def process_single_item(task_data, target_articles_map, reference_articles_map, criteria_map, 
                         llm_client, lock, pbar, max_retries, language, include_images=False):
    ...
    article_json = target_article_data.get('article_data', {})
    # article_text = article_json.get('text', '')
    article_text = article_json.get('clean_text', '')
    article_images = article_json.get('images', [])
    ...
    article_pad = "<|ARTICLE_PAD|>"

    # Prepare LLM prompt
    user_prompt = merged_score_prompt.format(
        task_prompt=prompt,
        article_1=article_pad,
        article_2=reference_article,
        criteria_list=criteria_list_str 
    )

    part1 = user_prompt.split(article_pad)[0]
    part2 = user_prompt.split(article_pad)[1]
    contents = []
    contents.append({"type": "text", "text": part1})

    text_segments = re.split(r'(<image>)', article_text)
    image_counter = 0
    
    for seg in text_segments:
        if seg == '<image>':
            if image_counter < len(article_images):
                b64_data = article_images[image_counter]
    ...
    contents.append({"type": "text", "text": part2})
    ...
    while retry_count < max_retries and not success:
        try:
            llm_response_str = llm_client.generate_message(
...
def process_language_data(language, target_model, llm_client, clean_agent, 
                         raw_data_dir, cleaned_data_dir, max_workers, limit, 
                         query_file, skip_cleaning, include_images=False):
    ...
    try:
        ...
        # --- Load target articles from generated report JSON files ---
        target_file_path = raw_data_dir
        target_articles_list = []
        
        # Match report_{id}.json files.
        json_files = glob.glob(os.path.join(target_file_path, 'report_*.json'))
        ...
        for file in json_files:
            try:
                with open(file, 'r', encoding='utf-8') as f:
                    ...
                    match = re.search(r'report_(\d+)\.json', file_name)
                    if match:
                        idx = int(match.group(1))
                        # Find the matching prompt from query.jsonl.
                        prompt = next((task['prompt'] for task in all_tasks if task.get('id') == idx), None)
                        
                        if prompt:
                            target_articles_list.append({
                                "id": idx,
                                "prompt": prompt,
                                "article_data": data
                            })
...
def main():
    global CRITERIA_FILE, REFERENCE_FILE
    ...
    parser.add_argument('--criteria_file', type=str, default=CRITERIA_FILE, help='Path to DRB criteria jsonl.')
    parser.add_argument('--reference_file', type=str, default=REFERENCE_FILE, help='Path to DRB reference jsonl.')
    parser.add_argument('--provider', type=str, default=DEFAULT_JUDGE_PROVIDER, choices=["uniapi", "siliconflow", "openai-compatible", "openai", "local", "vllm"], help='Judge provider.')
    parser.add_argument('--judge_model', type=str, default=DEFAULT_MODEL, help=f'Judge model name. Legacy SiliconFlow model: {DEFAULT_JUDGE_MODEL}')
    parser.add_argument('--api_url', type=str, default='', help=f'Full chat completions URL. SiliconFlow default: {DEFAULT_SILICONFLOW_API_URL}')
    parser.add_argument('--base_url', type=str, default=DEFAULT_UNIAPI_BASE_URL, help='OpenAI-compatible base URL.')
    parser.add_argument('--api_key_env', type=str, default='', help='Environment variable containing the judge API key. UniAPI tries UNIAPI_API_KEY then OPENAI_API_KEY by default.')
    parser.add_argument('--include_images', action='store_true', help='Send report images to the judge model in addition to text.')
    ...
    if not api_key and args.provider in {"local", "vllm"}:
        api_key = "EMPTY"
    if not api_key:
        raise ValueError(f"Judge API key missing. Checked env var: {used_api_key_env}")
    llm_client = AIClient(
        api_key=api_key,
        ...
        if not zh_tasks:
            ...
            if remaining_limit > 0:
                ...
                zh_results = process_language_data(
                    ...
                    raw_data_dir, cleaned_data_dir, max_workers, 
                    remaining_limit, query_file, skip_cleaning, include_images
                )
                if zh_results:
            ...
            zh_results = process_language_data( 
                "zh", target_model, llm_client, clean_agent,
                raw_data_dir, cleaned_data_dir, max_workers, 
                limit, query_file, skip_cleaning, include_images
            )
            if zh_results:
                all_results.extend(zh_results)
