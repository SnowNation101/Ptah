...
CMD=(
  ...
  --bench dc
  --report-dir "${REPORT_DIR:-outputs/dc}"
  --html-dir "${HTML_DIR:-outputs/dc}"
  --screenshot-dir "${SCREENSHOT_DIR:-output_screenshots/dc/ptah}"
  --start-id "${START_ID:-1}"
  --end-id "${END_ID:-10}"
  --limit "${LIMIT:-0}"
  --provider "$PROVIDER"
  --model "$MODEL"
  --base-url "$BASE_URL"
  --out-dir "${OUT_DIR:-eval_results/dc/ptaheval}"
)
